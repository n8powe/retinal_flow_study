...
for target=1:targetNumber
    ...
    tgDir = rand*2*pi;
    startPos = 0.5*screenRes;
    endPos = targetJump.*[cos(tgDir),sin(tgDir)]+0.5*screenRes;
%     tgPos = 
...
%     randPhase = rand*2*pi;
    ...
    timeStart = vbl;
    while GetSecs<(timeStart+durFix+durTarget)
        if (GetSecs-timeStart)<durFix
            currPos = startPos;
        else
            currPos = endPos;
        end
        ...
        Screen('FillRect', win, bg);
        for mm=1:markersNumber
            arucoCenter = currPos + arucoGapPix * [fix(mm/2.5)-0.5, rem(mm,2)-0.5];
        ...
        Screen('Drawtexture', win, stimTex, [],...
                [currPos-0.5*stimTex_rectScaled,currPos+0.5*stimTex_rectScaled]);
